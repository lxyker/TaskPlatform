...
def statistics_priority(request, project_id):
    ...
    result = models.Issues.objects.filter(
        project_id=project_id, create_datetime__gte=start, create_datetime__lt=end,
...
def statistics_project_user(request, project_id):
    ...
    all_user_dict[request.lxyker.project.creator.id] = {
        'name': request.lxyker.project.creator.username,
